<p align="center">
    <img src="screenshots/android_rss_grey.png" alt="RSS"/>
  </p>
...
- **Browser Privacy Controls** — fine-grained, in-app toggles for the WebView: cookie policy (block all / first-party only / allow all), tracker & ad blocking, JavaScript, site data (DOM storage), clear cookies & storage on close, "Do Not Track" / Global Privacy Control signals, Safe Browsing, and Force-HTTPS-only — privacy-preserving defaults out of the box
...
| WebView | 30+ tracker / ad domains blocked; SSL errors cancelled; HTTP auth blocked; popups blocked; camera / mic / location denied; `allowFileAccess=false`; mixed content forbidden; user-configurable cookie policy (first-party-only default), DNT / GPC signals, clear-on-close |
